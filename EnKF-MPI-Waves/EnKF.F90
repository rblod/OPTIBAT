...
!
! Modifications:
!                28/10/2011 FC: The code is adapted to work with oops
!                20/9/2011 PS:
...
program EnKF
  ...
  use distribute
  use mod_measurement
  use m_get_waves_nrens
  use m_get_waves_grid
  use m_get_waves_dim
  use m_obs
 ...
 ! use netcdf
  ...
  !real, allocatable, dimension(:,:) :: fld2
  ! Additional fields
...
!  if (iargc() /= 1) then
  ...
  allocate(nlobs_array(idm, jdm))
   ! get model grid
   !
   call get_waves_grid(modlon, modlat, depths, mindx, meandx, idm, jdm)
   if (master) then
...
!   allocate(fld2(idm * jdm, ENSSIZE * NFIELD))
