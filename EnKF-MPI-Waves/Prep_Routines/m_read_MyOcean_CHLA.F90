module m_read_MyOcean_CHLA
! Ehouarn !
! Reads CHLA1 data and grid from the GlobColour files !
contains

  subroutine read_MyO_CHLA(fname,gr,data)
  use mod_measurement
  use mod_grid
  !use m_datatest
  use m_spherdist
...
! Get dimension id in netcdf file ...
  ...
  call nfw_inq_dimid(trim(fname), ncid, 'latitude', NbLatitudes_ID)
  call nfw_inq_dimid(trim(fname), ncid, 'longitude', NbLongitudes_ID)
...
! Variable ids in netcdf file
  call nfw_inq_varid(trim(fname),ncid,'latitude' ,vNbLatitudes_ID)
  call nfw_inq_varid(trim(fname),ncid,'longitude' ,vNbLongitudes_ID)
  call nfw_inq_varid(trim(fname),ncid,'chl' ,vGrid0001_ID)
...
end subroutine read_MyO_CHLA

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

subroutine read_MyO_CHLA_grid(filename,gr)
  !use mod_dimensions
  use mod_grid
  use netcdf
  use nfw_mod
...
! Get dimension id in netcdf file ...
  call nfw_inq_dimid(trim(filename), ncid, 'latitude', NbLatitudes_ID)
  call nfw_inq_dimid(trim(filename), ncid, 'lontitude', NbLongitudes_ID)
...
! Variables in NetCDF file
  ...
  call nfw_inq_varid(trim(filename),ncid,'latitude' ,vNbLatitudes_ID)
  call nfw_inq_varid(trim(filename),ncid,'longitude' ,vNbLongitudes_ID)
  ...
  call nfw_get_att_double(trim(filename),ncid,vNbLatitudes_ID ,'step',dlatlon(1))
  call nfw_get_att_double(trim(filename),ncid,vNbLongitudes_ID,'step',dlatlon(2))
...
!  gr%dx=   0.179
 ...
 end subroutine read_MyO_CHLA_grid
 
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 
 
end module m_read_MyOcean_CHLA
