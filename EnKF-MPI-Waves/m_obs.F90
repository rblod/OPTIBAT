...
module m_obs
  ...
  real, parameter, private :: STR_MIN = -100.0
  real, parameter, private :: STR_MAX = 100.0
  real, parameter, private :: EPB_MIN = -100.0
  real, parameter, private :: EPB_MAX = 100.0
  real, parameter, private :: OCG_MIN = -100.0
  real, parameter, private :: OCG_MAX = 100.0
...
contains
  ...
  subroutine obs_testrange
    ...
    do uo = 1, nuobs
       if (trim(unique_obs(uo)) == 'STR') then
          dmin = STR_MIN
          dmax = STR_MAX
       elseif (trim(unique_obs(uo)) == 'EPB') then
          dmin = EPB_MIN
          dmax = EPB_MAX
       elseif (trim(unique_obs(uo)) == 'OCG') then
          dmin = OCG_MIN
          dmax = OCG_MAX  
       else
          dmin = -1.0d6
