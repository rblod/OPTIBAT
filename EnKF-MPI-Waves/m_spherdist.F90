...
real function spherdist(lon1, lat1, lon2, lat2)
  ...
  spherdist = REARTH * acos(sin(rlat1) * sin(rlat2)&
       + cos(rlat1) * cos(rlat2) * cos(rlon1 - rlon2))
end function spherdist

real function eucldist(lon1, lat1, lon2, lat2)
  implicit none

  real(8), intent(in) :: lon1, lat1, lon2, lat2 ! pos. in m (center of the cell)
  
  eucldist=sqrt((lon1-lon2)**2+(lat1-lat2)**2) 
