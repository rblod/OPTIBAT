...
! ehouarn ETKF    
    real, dimension(:), allocatable::work,sig
    real, dimension(:,:),allocatable::U,V
    integer::lwork,k,nrsigma
    real::sigsum,sigsum1
    real, allocatable, dimension(:,:) :: X1tmp 
    
    if (trim(METHODTAG) == "ENKF") then
       SCHEME_USED = SCHEME_ENKF
    elseif (trim(METHODTAG) == "DENKF") then
       SCHEME_USED = SCHEME_DENKF
    elseif (trim(METHODTAG) == "ETKF") then
       SCHEME_USED = SCHEME_ETKF  
    end if
    ...
    if (master) then
       ...
       elseif (SCHEME_USED == SCHEME_ETKF) then
          print *, 'using ETKF analysis scheme'  
       end if
    end if
    ...
    do jj = my_first_iteration, my_last_iteration
	  ...
	  if(SCHEME_USED == SCHEME_ETKF) then
             allocate(X1tmp(nrens, nrens))
	     !ehouarn
          end if
          ...
          if (LOCFUN_USED /= LOCFUN_NONE) then
             do o = 1, nlobs
                obs0 = obs(lobs(o))
		!ehouarn
                dist = eucldist(modlon(i, j), modlat(i, j),&
                     obs0 % lon, obs0 % lat)
		!dist = spherdist(modlon(i, j), modlat(i, j),&
          ...
          do iter = 1,2
             ...
             if ((nlobs < nrens).and.(SCHEME_USED /= SCHEME_ETKF)) then ! use observation space
	  ...
	  elseif (SCHEME_USED == SCHEME_ETKF) then
	     ! X1=inv(I+S'*S) 
	     allocate(U(nrens,nrens),V(nrens,nrens),sig(nrens))
	     lwork=10*nrens
	     allocate(work(lwork))
             call dgesvd('A','A', nrens,nrens,X1,nrens,sig,&
	     	          U,nrens,V,nrens,work,lwork,lapack_info)
	     deallocate(work,V)	  
	     if (lapack_info /= 0) then
                print *, '  ERROR: m_local_analysis(): LAPACK error in dgesvd: errno = '&
                        , lapack_info, 'i, j =', i, j
                call stop_mpi
             endif
	     ...
	     do k=1,nrens
	       do m=1,nrens
	          X1tmp(k,m)=U(k,m)*sqrt(sig(m))
	       enddo	
	     enddo

	     X5tmp = X5tmp + matmul(X1tmp,transpose(U)) 
	     
	     deallocate(X1tmp,sig,U)
          end if
  ...
  subroutine get_local_obs(i, j, rmax, modlon, modlat, mindx,&
    ...
    do o = 1, nobs
       ...
       !ehouarn
       dist(o) = eucldist(obs(o) % lon, obs(o) % lat, modlon(i, j), modlat(i, j))
