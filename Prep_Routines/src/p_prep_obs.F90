...
program p_prep_obs
  ...
  use m_uobs
  use m_get_micom_dim
  use m_read_SOCAT_gridded
  use m_read_GlobC_CHLA
  use m_read_OCCCI
  use m_read_HAMOCC_CHLA
  use m_read_MyOcean_CHLA
  implicit none
  ...
  elseif (trim(producer) == 'SOCAT') then
    !ehouarn: fCO2 real data
      if (trim(obstype) == 'FCO2') then
        dosuperob = .true.
        read(fname,'(i2)') nmonth
	print*,'nmonth= ', nmonth, trim(fname)
	call read_SOCAT_grid(fnamehdr,gr)
	grpoints=gridpoints(gr)
	allocate(data(grpoints))
        allocate(obs(maxobs))
	call read_SOCAT_FCO2(fnamehdr,nmonth,gr,data)
      elseif (trim(obstype) == 'AFCO2') then
        dosuperob = .true.
        read(fname,'(i2)') nmonth
	print*,'nmonth= ', nmonth, trim(fname)
	call read_SOCAT_grid(fnamehdr,gr)
	grpoints=gridpoints(gr)
	allocate(data(grpoints*lasswin))
	!allocate(data(grpoints*12))
        allocate(obs(maxobs))
	!call read_SOCAT_FCO2_asynchro(fnamehdr,nmonth,gr,data,12)
	call read_SOCAT_FCO2_asynchro(fnamehdr,nmonth,gr,data,lasswin)
	gr%nx=gr%nx*lasswin
	!gr%nx=gr%nx*12 ! a verifier pourquoi?
      else
        print *, 'data of type "', trim(obstype),'"  from producer "', producer, '" is not handled'
        stop  'ERROR: p_prep_obs'
      endif
  elseif (trim(producer)=='GlobColour')then
      if (trim(obstype)=='CHLA') then
        dosuperob = .true.
	call read_CHLA_grid(fnamehdr,gr)
        grpoints=gr%nx*gr%ny
        allocate (data(grpoints))
        allocate (obs(maxobs)) 
        call read_CHLA(fname,gr,data)
      else
        print*,'No ',obstype,' data from:',producer
        stop  
      endif
  elseif (trim(producer)=='OCCCI')then
      if (trim(obstype)=='CHLA') then
        dosuperob = .true.
	call read_OCCCI_singrid(fnamehdr,gr)
        grpoints=gr%nx
        allocate (data(grpoints))
        allocate (obs(maxobs)) 
        !call read_OCCCI_CHLA_binned(fname,gr,data)
        call read_OCCCI_CHLA_sin(fname,gr,data)
      else
        print*,'No ',obstype,' data from:',producer
        stop  
      endif
   elseif (trim(producer)=='MyOcean')then
      if (trim(obstype)=='ACHLA') then
        dosuperob = .true.
	call read_MyO_CHLA_grid(fnamehdr,gr)
        grpoints=gr%nx*gr%ny
        allocate (data(grpoints))
        allocate (obs(maxobs)) 
        call read_MyO_CHLA(fname,gr,data)
      else
        print*,'No ',obstype,' data from:',producer
        stop  
      endif   
  elseif (trim(producer) == 'HAMOCC') then
   !ehouarn: twin exp
      if (trim(obstype)=='ACHLA') then
        dosuperob = .false.
	nechx=2
	nechy=2
        !allocate(mask3d(nx,ny,12))
	allocate(mask3d(nx,ny,lasswin))
	
	!call get_CHLA_mask(fname,nx,ny,12,depths,mask3d,grpoints)
	call get_CHLA_mask(fname,nx,ny,lasswin,depths,mask3d,grpoints)
	allocate(data(grpoints))
	
	!call read_hamocc_ACHLA(fnamehdr,data,mask3d,nx,ny,12,grpoints,nrobs,modlon,modlat)	
        call read_hamocc_ACHLA(fnamehdr,data,mask3d,nx,ny,lasswin,grpoints,nrobs,modlon,modlat)
	deallocate(mask3d) 
	data_eq_obs = .true.
      ...
      elseif (trim(obstype) == 'AFCO2') then
        ...
        read(fname,'(i2)') nmonth
	print*,'nmonth= ', nmonth, trim(fname)
	!fnamehdr2='SOCAT_tracks_gridded_monthly.nc'
	
	grpoints=get_nobs_SOCAT_TW(nx,ny,lasswin)
	print*,'nb obs in HAMOCC: ',grpoints
        !allocate(mask1d(1:grpoints))
	allocate(data(grpoints))
	allocate(noise1d(grpoints))
        
	call get_obs_SOCAT_TW(data,grpoints,nx,ny,lasswin)
	call compute_noise_SOCAT(noise1d,data,grpoints)
        call read_hamocc_AFCO2(fnamehdr,data,nx,ny,grpoints,noise1d,nrobs)
	...
	call get_SST_mask(fname,nx,ny,depths,mask2d,grpoints)
        allocate(data(grpoints))
	call read_hamocc_SST(fnamehdr,data,mask2d,nx,ny,grpoints,nrobs,modlon,modlat)	
	deallocate(mask2d)
  ...
  if (.not. data_eq_obs) then
     ...
     !Fanf: we want dat next to land as well
     !call check_forland(data, depths, size(data), nx, ny)
      print*,'nrobs eho',nrobs
     allocate(obs(nrobs))
     k=1
     do i = 1, grpoints
