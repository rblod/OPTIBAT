...
for i in range(100):
	h=ncf_get_var_t('ana_cat_'+str(num)+'.nc','h',i)
	Cg=ncf_get_var_t('ana_cat_'+str(num)+'.nc','Cg',i)
	plt.plot(-h,Cg,'k+')
ax.set_xlabel('h')	
ax.set_ylabel('Cg')	
ax.set_title('Analysis at step '+str(num))

fig=plt.figure(3)
ax = fig.add_subplot(111)
for i in range(100):
	h=ncf_get_var_t('ana_cat_'+str(num)+'.nc','h',i)
	epb=ncf_get_var_t('ana_cat_'+str(num)+'.nc','epb',i)
	plt.plot(-h,epb,'k+')
ax.set_xlabel('h')	
ax.set_ylabel('epb')	
ax.set_title('Analysis at step '+str(num))
